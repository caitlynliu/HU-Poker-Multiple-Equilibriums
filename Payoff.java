...
public class Payoff {
    ...
    double handProb;
    public Payoff(double stack) throws FileNotFoundException, IOException{
...
*/  
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException {
        double[] stackSizes = {20.0,1000.0};
        double stack;
        for(int i =0;i<stackSizes.length;i++){
            stack = stackSizes[i];
            Payoff payoff= new Payoff(stack);
            FileWriter csvWriter =new FileWriter("payoff"+stack+".csv");
            for(int j=0;j<1326;j++){
                for(int k = 0; k<1326; k++){
                    csvWriter.append(Double.toString(payoff.getPayoff(j,k)));
                    csvWriter.append(",");
                }
                csvWriter.append("\n");
            }
